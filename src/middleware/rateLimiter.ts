...
const rateLimiter = (req:Request,res:Response,next:NextFunction)=>{
	...
	if(memoryStore[clientIP].count > rateLimitConfig.max){
		const timeRemaining = Math.ceil((memoryStore[clientIP].expiry - Date.now()) / 1000)
		res.status(429).send(`To many requests from this IP please try again later. ${timeRemaining}`)
	}
